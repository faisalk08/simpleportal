...
FileListener.prototype.syncFolder = function(listenDir, destDir, eventprops){
	var instance = this;
	console.log('filelistener:syncFolder:>' + listenDir  + ">>>>>" + destDir);
	...
	instance.listen(listenDir, function(listnerdata){
		console.log('filelistener:syncFolder:['+listenDir+']:>' + listnerdata.listenDir + "/" + listnerdata.filename + ">>>>>" + destDir  + "/" + listnerdata.filename);
...
function moveFile (srcdir, destdir, filename, eventprops, callback){
	console.log('filelistener:moveFile:['+filename+']:>' + srcdir + ' >> '+ destdir);
	...
	if(!fs.existsSync(srcfile)){
		...
		if(fs.existsSync(destfile) && fs.statSync(srcfile).mtime <= fs.statSync(destfile)){
			console.log('filelistener:moveFile:' + destfile + ' - file already moved.');
...
var watchFolder = function(folderToWatch, callback){
    console.log('filelistener:', 'watchFolder:::' + folderToWatch);    
	try{
		fs.watch(folderToWatch, {recursive:true}, function (event, filename) {
	  		console.log('filelistener:watchFolder:', filename + ':> event[' + event + ']');
	  		if (filename) {
	    		if(callback){
	    			callback(filename);
	    		}
	  		} else {
	    		console.warn('filelistener:watchFolder:', 'filename not provided');
	  		}
		});	
	}catch(error){ 
		console.log(error);
