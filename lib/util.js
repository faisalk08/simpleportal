...
util.sendServiceResponse = function(response, error, results, headers){
	...
	if(error){
		logger.getInstance().error('Simple Portal -service - util', error);
	...
	if(error && error == 'Permission denied'){
		...
		logger.getInstance().error('Simple Portal -service - util', 'Login required');
		var httpstatus = 301;
...
util.generateId = function(name) {
	if (name) {
		...
		//logger.getInstance().info('Simple Portal -service - util', 'Util generate id is : '+ name);
	}
