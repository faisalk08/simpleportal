            ...
            if(typeof module == 'function'){
                ...
                for(var function_ in module){
                    if(typeof module[function_] == 'string' || typeof module[function_] == 'function'){
                            ...
                            util.callModuleFunction(module[function_], true, functionName, args, args1);
                    }
                }
            }
    }
...
util.sendServiceResponse = function(response, error, results){
	if(error){
		...
		response.send(400, {'content-type': 'application/json; charset=UTF-8'}, JSON.stringify({exception:error}));
		...
		response.send(200, {'content-type': results.contentType}, results.data);
	} else {
...
util.getDayDifference=function(startDate, endDate) {
