...
//	simpleportalOAuth = require('./oauth'),
...
//    var cookies;
//	if(server_options.cookies) {
//		cookies = server_options.cookies;
//		
//		delete server_options.cookies;
//	}
    ...
    if(request && server_options.oauthprovider&&!oauthdata){
    	...
    	if(server_options.oauthprovider && server_options.skiplogin){
    		...
    		if(simpleportaloauth){
    			...
    			callback('Not a valid oauth provider, check your oauth configuration');
    	}  else if(userprofile && userprofile.oauth && userprofile.oauth[/*server_options.oauthprovider + */'esystemandroid']){
    		var simpleportaloauth = simpleportalOAuth.getSimplePortalOAuth(/*server_options.oauthprovider + */'esystemandroid');
    		
    		if(simpleportaloauth){
    			simpleportaloauth.getJSON(server_options, request, callback, userprofile.oauth[/*server_options.oauthprovider + */'esystemandroid']);
    		} else 
    			callback('Not a valid oauth provider, check your oauth configuration');
    	}else if(userprofile){
    		var simpleportaloauth = simpleportalOAuth.getSimplePortalOAuth(server_options.oauthprovider);
...
//		        		if(source.hasOwnProperty(prop) && target.hasOwnProperty[prop] && source[prop] + '' == "true" && target[prop] + '' == "false")
//		        			target[prop] = false;
//		        		else{
		        			var targetvalue;
...
util.downloadFile =  function(url, path, callback) {
	...
	console.log('Simpleportal:util:downloadFile', 'Downloading file from remote server -- ' + url);
	console.log('Simpleportal:util:downloadFile', 'Downlaoded file will be moved to  -- ' + path);
...
/**
 * To append a file extension, only if the existing path not ends with the extension
 * @method appendFileExtension
 * @param path
 * @param extension extension to be appeneded to the path 
 * 
 */
util.appendFileExtension = function(path, extension){
	return path + ((path.indexOf(extension) + extension.length == path.length) ? "" : extension);
}

/**
 * Appending a file path if the exisiting path contains the apending path, array of paths can be appended with file seperator
 * @method appendFilePath
 * @param target path where list of sub paths to be appeneded
 * 
 */
...
//	if(modelprops && typeof modelprops == "object" && modelprops.getConfiguration("modelsettings")){
//		console.log(modelprops.getConfiguration("modelsettings"));
//	}
...
//		console.log("object _id  >> " + _id);
		try{
			...
			object._id=_id;
			console.log("Wrong _id value provided - " + _id)
		}
	}
//	console.log(object);
