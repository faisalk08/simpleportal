...
util.sendServiceResponse = function(response, error, results){
	...
	if(error && error == 'Permission denied'){
		logger.error('Simple Portal -service - util', error);
		response.send(403, {'Content-Type': 'application/json; charset=UTF-8'}, body);
	} else if(error){
		logger.error('Simple Portal -service - util', error);
		...
		response.send(httpstatus, { 'Location': '/oauth/login'});
	} else if(results && results.redirectUrl){
		var httpstatus = 301;
		if(results.httpstatus)
			httpstatus = results.httpstatus;
		response.send(httpstatus, { 'Location': results.redirectUrl});
	} else if(results && results.contentType){
