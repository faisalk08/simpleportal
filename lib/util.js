...
var simpleportalOAuth = require('./oauth');
...
util.post = function(server_options, request, callback, oauthdata){
    ...
    if(request && server_options.oauthprovider){
    	...
    	if(userprofile&&userprofile.oauth&&userprofile.oauth[server_options.oauthprovider]){
    		...
    		if(simpleportaloauth){
    			simpleportaloauth.post(server_options, request, callback, oauthdata);
    		} else
    ...
    if(request && server_options.oauthprovider&&!oauthdata){
        ...
        if(request.session.oauth && request.session.oauth.loggedIn){
            ...
            if(request.session.oauth.oauthquery){
                ...
                var sign_url = oa.signUrl(path, oauthdata.access_token, oauthdata.access_token_secret, (server_options.method||"GET"));
                var parsedUrl= url.parse(sign_url, false );
...
util.extendJSON = function(target) {
    ...
    sources.forEach(function (source) {
    	if( typeof target == 'object' && typeof target.length == 'number' && typeof source == 'object' && typeof source.length == 'number'){
    		target.concat(source);
    	}else
	    	for (var prop in source) {
	    		if( typeof target[prop] == 'object' && typeof target[prop].length == 'number' && typeof source[prop] == 'object' && typeof source[prop].length == 'number')
