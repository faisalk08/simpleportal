...
var CRUDService = function(options) {
    ...
    if(instance.collection){
        instance.service['getLocalCache'] = function(){
            ...
            localCache = new db.LocalCache(dbInstance, 'session');
            return localCache;
        }
    }
    ...
    if(instance.service['getLocalCache']){
    	...
    	logger.warn('CRUDService Wrapper', this + 'No method for getLocalCache');
    }
};
...
CRUDService.prototype.registerR = function(){
    ...
    if(service.beforeDetailsCallback){
    	service.details = function(id, callback){
            if (id) {
                service.getLocalCache().findOne({id: id}, function(error, object) {
                    if (!object || object == '')
    	...
    	service.details = function(id, callback){
            if (id) {
                service.getLocalCache().findOne({id: id}, function(err, object) {
                    if (!object || object == '')
                        callback("There is no result found for the id " + id);
                    else
