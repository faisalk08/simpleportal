...
var LocalCache = function(dbInstance, collectionName) {
	...
	instance.getCollection(function(error, data){
		console.log(error);
		if(error)
			logger.warn('DB ', 'Some problem while getting collection from db -' + error);
		else
			logger.info('DB ', 'Collection retreived from db successfully');
	});
};
...
LocalCache.prototype.save = function(results, callback){
	var instance = this;
	if(!results){
		logger.warn('DB ', 'No result skipping save!!');
