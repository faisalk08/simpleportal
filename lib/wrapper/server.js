...
/**
 */
Server.prototype.stopNodeServer = function(modulePath, callback, returndata){
	var instance = this,
		pid,
		pidfile = path.dirname(modulePath) + '/pid';
	...
	if(fs.existsSync(pidfile)){
		...
		if(require("is-running")(pid)){
			...
			callback("Server is not running pid -" + pid);
	}else{
		callback("no process id found for your server.");
	}
};

Server.prototype.forkNodeServer = function(modulePath, callback, returndata){
	var instance = this,
		pid,
		pidfile = path.dirname(modulePath) + '/pid';
	...
	console.log("Starting new server >" + modulePath);
	
	if(fs.existsSync(pidfile))
		pid = fs.readFileSync(pidfile)+ '';
	
	if(require("is-running")(pid)){
		callback("Server allready running with process id -" + pid);
	}else{
...
//		if(loggerconfig.root.indexOf(".") == 0 || loggerconfig.root.indexOf("/") != 0)
		loggerconfig.root = simpleportal.util.getServerPath(loggerconfig.root);
	...
	if(request.publicpagelayout){
		var templateOption =  {
			...
			data:simpleportal.util.extendJSON({
				...
				serverconfig:instance.getConfiguration(),
				systemconfig:simpleportal.getConfiguration(),
