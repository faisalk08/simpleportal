...
Server.prototype.configureconnect = function() {
	...
	if(simpleportal.filterloader.filters.length != 0){
		for(var i = 0; i < simpleportal.filterloader.filters.length;i++){
			...
			var filtername = simpleportal.filterloader.filters[i];
			...
			if(filterconfig && filterconfig.disabled)
				logger.getInstance().info('Simpleportal - Server:configureconnect' , 'DISABLED  --- FILTER - ' + filtername);
			else{
				logger.getInstance().info('Simpleportal - Server:configureconnect' , 'FILTER - ' + filtername);
				
				instance.server.use(new require(simpleportal.util.getServerPath(simpleportal.filterloader.filterdir + '/' +filtername + '.js', true))(filterconfig));
			}
		}
	}
...
Server.prototype.configure = function() {
	...
	else if(fs.existsSync('./node_modules/simpleportal/default_/resources/public/simpleportal/favicon.ico'))
		instance.server.use(connect.favicon('./node_modules/simpleportal/default_/resources/public/simpleportal/favicon.ico'));
...
Server.prototype.startServer = function() {
	...
	try{
		...
		if(fs.existsSync('./node_modules/simpleportal/default_/resources/public'))
			instance.server.use(connect.static('./node_modules/simpleportal/default_/resources/public'));
...
Server.prototype.createServer = function(options) {
	...
	//Autocreate folders
	if(options&&!options.disableautocreate){
		util.checkDirSync(getServerPath('api'));
		util.checkDirSync(getServerPath('view'));
		util.checkDirSync(getServerPath('filter'));
		
		util.checkDirSync(getServerPath('resources/public'));
		util.checkDirSync(getServerPath('resources/plugin'));
	}	
