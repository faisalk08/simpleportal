...
Server.prototype.configure = function() {
	 ...
	 */
	if(configuration.resources.theme){
		var themesDir = simpleportal.util.getServerPath(configuration.resources.theme.root || (staticResourceDir ? staticResourceDir : 'public' ) + '/themes');
		var rootThemeDir = themesDir+ '/' + (configuration.resources.theme.use || 'theme');
		...
		if(fs.existsSync(themesDir)){
			...
			if(themes.length == 1 || (!configuration.resources.theme.use && themes.length > 1)){
				instance.server.use('/theme', connect.static(themesDir + '/' + themes[0]));
				
				if(!configuration.resources.theme.use && themes.length > 1)
