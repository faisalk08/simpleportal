...
Server.prototype.init = function() {
	...
	instance.on('server.init', function(){
		...
		if(instance.options['mode-configuration'] && !configserver){
			var configport = Number(instance.configuration.port) + 10;
			logger.getInstance().info('Simple Portal -server', 'Creating Config sever on - '+ configport);
			
			configserver = new simpleportal.Configserver({port:configport}, instance);
	...
	instance.on('dispatch.post', function(){
		...
		console.log(instance.options['resources-simpleportal'] && fs.existsSync(staticResourceDir + 'simpleportal'));
		console.log(instance.options['resources-simpleportal'] );
		console.log(instance.options['resources-simpleportal'] && fs.existsSync(__dirname + '/../../resources') );
