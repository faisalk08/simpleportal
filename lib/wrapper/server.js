...
Server.prototype.init = function() {
	...
	instance.on('dispatch.post', function(){
		 ...
		 *  configuration.theme
		 *  use -- default theme
		 *  root -- default /public/themes 
		 *//*
		if(instance.configuration.resources.theme){
			var themesDir = simpleportal.util.getServerPath(instance.configuration.resources.theme.root || (staticResourceDir ? staticResourceDir : 'public' ) + '/themes');
			var rootThemeDir = themesDir+ '/' + (instance.configuration.resources.theme.use || 'theme');
			
			// create themes router
			if(fs.existsSync(themesDir)){
				// now check it is only one theme
				var themes = fs.readdirSync(themesDir).filter(function(file) {
					return fs.statSync(path.join(themesDir, file)).isDirectory();
				});
				
				if(themes.length == 1 || (!instance.configuration.resources.theme.use && themes.length > 1)){
					instance.server.use('/theme', connect.static(themesDir + '/' + themes[0]));
					
					if(!instance.configuration.resources.theme.use && themes.length > 1)
						logger.getInstance().warn('Simpleportal - Server:configure', 'Using first theme from themes folder, No theme defined in configuration please configure [theme.use] property');
				} else if (fs.existsSync(rootThemeDir))
					instance.server.use('/theme', connect.static(rootThemeDir));
				else
					logger.getInstance().warn('Simpleportal - Server:configure', 'Defined theme directry not found ' + rootThemeDir);
				
				instance.server.use('/themes', connect.static(themesDir));
			}else
				logger.getInstance().warn('Simpleportal - Server:configure', 'Defined theme directry not found ' + themesDir);
		}*/
	});
...
Server.prototype.startServer = function() {
	...
	if(instance.serverstatus != 'running')
		try{
			...
			if(/*instance.options['configuration-editable'] || */instance.options['mode-configuration'] && !serverconfigeditor){
				var editoroptions = {
				...
				var themes = simpleportal.pluginloader.getPlugins('theme');
				var themes_= [];
				...
				if(themes_ && themes_.length > 0)
					var extendedFields =  [
	                    { html:{caption:'Default theme'}, field: 'resources__theme__use', type:'list', options:{items:themes_}}
					];
				
				extendedFields =  [
                    { html:{caption:'Add New Oauth provider'}, field: 'oauth__new', type:'new', parentfield:'oauth', objecttype:'object'}
				];
				
				var oauthproviders = Object.keys(simpleportal.oauth.getOauthroviders(instance.configuration));
				if(oauthproviders && oauthproviders.length > 0)
					extendedFields.push({ html:{caption:'Oauth provider to use'}, field: 'oauth__use', type:'list', options:{items:oauthproviders}});
				
				var oauthfields = Object.keys(simpleportal.Oauth.SimplePortalOAuth.DEFAULT_PROPS);
				
				for(oindex in oauthproviders){
					var oauthExtendedfields = editorUtil.getFieldFromObject(simpleportal.Oauth.SimplePortalOAuth.DEFAULT_PROPS, "oauth__" + oauthproviders[oindex] + "__");
					
					for(var fieldIndex in oauthExtendedfields){
						var oauthExtendedfield = oauthExtendedfields[fieldIndex];
						if(fieldIndex == 0)
							oauthExtendedfield.html.category = 'Oauth Configuration for - ' + oauthproviders[oindex];
						
						extendedFields.push(oauthExtendedfield);
					}
				}
				 ...
				 */
				var filters = [];
				for(findex in simpleportal.filterloader.filters){
					var filterprops = simpleportal.filterloader.filters[findex];
					
					var filterExtendedfields = editorUtil.getFieldFromObject(simpleportal.filterloader.DEFAULT_PROPS, "filter__" + filterprops.name + "__");
					for(var fieldIndex in filterExtendedfields){
						var filterExtendedfield = filterExtendedfields[fieldIndex];
						if(fieldIndex == 0)
							filterExtendedfield.html.category = 'Filter Configuration for - ' + filterprops.name;
						
						extendedFields.push(filterExtendedfield);
					}
				}
				...
				var webapps_ = [];
				for(findex in webapps){
					var webappprops = webapps[findex];
					
					var webappExtendedfields = editorUtil.getFieldFromObject(simpleportal.pluginloader.DEFAULT_PROPS, "webapp__" + webappprops.id + "__");
					for(var fieldIndex in webappExtendedfields){
						var webappExtendedfield = webappExtendedfields[fieldIndex];
						if(fieldIndex == 0)
							webappExtendedfield.html.category = 'Webapp Configuration for - ' + webappprops.title;
						
						extendedFields.push(webappExtendedfield);
					}
				}
				...
				//EDITOR OPTIONS WITH fields
				editoroptions.fields = extendedFields;
				
				serverconfigeditor = new ServerConfigEditor(editoroptions, instance.configuration);
				
				serverconfigeditor.route(instance.server);
				
				// Activate New oauth provider
				serverconfigeditor.registerSubEditor('oauth', simpleportal.Oauth.SimplePortalOAuth);
				
				//serverconfigeditor.registerSubEditor('webapp', simpleportal.pluginloader);
				//serverconfigeditor.registerSubEditor('filter', simpleportal.filterloader);
...
Server.prototype.configureconnect = function() {
	...
	if(instance.options['mode-configuration']){
		...
		themes.forEach(function(themesetting){
			instance.loadPluginURI(themesetting);
		});
	}
}
...
Server.prototype.loadPluginURI = function(pluginsetting, callback){
	...
	if(pluginsetting.plugintype=='webapp'){
		...
		if(pluginsetting.installeddir && pluginsetting.installed && fs.existsSync(pluginsetting.installeddir))
			instance.server.use('/theme' + pluginsetting.themeuri, connect.static(pluginsetting.installeddir));
		
		if(instance.configuration.resources.theme.use && pluginsetting.id == instance.configuration.resources.theme.use)
			instance.server.use('/theme', connect.static(pluginsetting.installeddir));
		else{
			instance.configuration.resources.theme.use=pluginsetting.id;
			instance.server.use('/theme', connect.static(pluginsetting.installeddir));
		}	
	}
