...
Server.prototype.configure = function() {
	 ...
	 */
	console.log(configuration.resources);
	
	if(configuration.resources.theme){
		var themeDir = simpleportal.util.getServerPath(configuration.resources.theme.root || (staticResourceDir ? staticResourceDir : 'public' ) + '/themes');
		var rootThemeDir = themeDir+ '/' + configuration.resources.theme.use || 'theme';
		
		// create themes router
		if(fs.existsSync(themeDir)){
			instance.server.use('/themes', connect.static(themeDir));
			
			// now check it is only one theme
			var themes = fs.readdirSync(themeDir).filter(function(file) {
				return fs.statSync(path.join(themeDir, file)).isDirectory();
			});
			
			if(themes.length == 1)
				instance.server.use('/theme', connect.static(themeDir + '/' + themes[0]));
