 ...
 */
SimplePortalOAuth.prototype.getOAuthObjct = function(accessdata){
	var instance = this;
	...
	if(accessdata){
		...
		if(accessdata.access_token_secret)
 ...
 */
SimplePortalOAuth.prototype.getUrl = function(){
	var instance = this;
	
	// server_option
	return (instance.configuration.secure ? "https://" : "http://") + instance.configuration.oauthHost + ((instance.configuration.oauthPort == 443||instance.configuration.oauthPort == 80) ?  "" : instance.configuration.oauthPort);
}

SimplePortalOAuth.prototype.processUrl = function(server_options, request, accessdata){
	// server_option
	var instance = this;
	var oAuthObject = instance.getOAuthObjct();
	...
	if(request){//@TODO logged in user can update the access token ??
		if(request.session)
			userprofile = request.session.userprofile;
		else
			userprofile = request.userprofile;
		
		if(userprofile && userprofile.oauth && userprofile.oauth[oAuthObject.oauthproviderkey]){
    		accessdata = userprofile.oauth[oAuthObject.oauthproviderkey];
    	} else if(!accessdata && request.session.oauth && request.session.oauth.oauthproviderkey == oAuthObject.oauthproviderkey){
    		accessdata = {oauthprovider:request.session.oauth.oauthproviderkey, access_token:request.session.oauth.access_token, access_token_secret:request.session.oauth.access_token_secret}
    	}
	}
	
	var server_path = util.constructUrl(server_options);
    var sign_url = oAuthObject.signUrl(server_path, accessdata.access_token, accessdata.access_token_secret, (server_options.method||"GET"));
...
SimplePortalOAuth.prototype.post = function(server_options, request, callback, accessdata){
	...
	var instance = this;
	instance.processUrl(server_options, request, accessdata);
...
SimplePortalOAuth.DEFAULT_PROPS={
	"secure":false,
	"oauthHost":"localhost",
	"oauthPort":"80",
	...
	"consumerSecret":"",
	"version":"1.0",
	"authorizeCallback":"/oauth/callback",
	"signatureMethod":"HMAC-SHA1",
	"loginUrl":"/oauth/login",
	"logoutUrl":"/oauth/logout",
	"successUrl":"/oauth/status",
	"authorizeUrl":"/oauth/authorize",
	"authorizedAccessUrl":"/oauth/authorized_token"
};
