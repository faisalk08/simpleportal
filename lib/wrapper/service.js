...
Service.prototype.getObject = function(request){
	...
	var object = {};
	logger.getInstance().debug('Simpleportal -Service', 'getting object ');
	...
	logger.getInstance().debug('Simpleportal - service wrapper', object);
	logger.getInstance().debug('Simpleportal - service wrapper', validationmessages);
...
Service.prototype.init = function(configuration, dbInstance){
	...
	var simpleportal = require('simpleportal');
	logger.getInstance().info('Service Wrapper', 'Initializing service - ' + instance.name);
    ...
    if(instance.collection){
        ...
        if(!instance.service['POST /'])
			instance.post('/', function(request, response, callback) {
				...
				/*
				logger.getInstance().debug('Simpleportal - Service', 'we are getting the object....');
				logger.getInstance().debug('Simpleportal - Service', object);
...
Service.prototype.servicepre = function(request, response, servicecallback, callback){
	...
	}else if(instance.userrole && (instance.userrole != request.user.role) ){
		callback('Permission denied');
	}else{
