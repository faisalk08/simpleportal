...
DynamicPluginAppRouter.prototype.updateOptions = function(request){
	...
	if(request.webpage) {} 
		...
		(
			...
			console.log("search for layout using page id --> " + request.url);
			var webpage;
			var pageid = request.url.replace("/pages", "");
			...
			if(self.viewsettings.webappsetting && self.viewsettings.webappsetting.websiteconfig.webpages) {
				...
				if(!webpage)
					webpage = util.getJSONObject(self.viewsettings.webappsetting.websiteconfig.webpages, 'uri', pageid);

				if(!webpage)
					webpage = util.getJSONObject(self.viewsettings.webappsetting.websiteconfig.webpages, 'uri', pageid.replace(".html", ""));
			}
			
			if(webpage){
				request.webpage = webpage;
				if(!request.webpage.layout)
