...
var PluginLoaderMain = module.exports = function(options, serverInstance){
	...
	instance.systemplugindir = options.systemplugindir || path.join(simpleportal.getRootdir(), "server/resources/plugins");
	instance.systemuiplugindir = options.systemuiplugindir || path.join(simpleportal.getRootdir(), 'server/resources/uiplugin');

	/*
	 * Configuration for loading various plugins
	 * uiplugin, util webapp,theme
	 */
	instance.publicappdir = simpleportal.util.getServerPath(options.publicappdir||"resources/public");
	instance.plugindir = simpleportal.util.getServerPath(options.plugindir || 'resources/plugin');
	instance.uiplugindir = simpleportal.util.getServerPath(options.uiplugindir ||'resources/uiplugin');
	instance.templatedir = simpleportal.util.getServerPath(options.templatedir || path.join(simpleportal.getRootdir(), 'server/resources/templates/plugin'));
	instance.layoutdir = simpleportal.util.getServerPath(options.layoutdir || path.join(simpleportal.getRootdir(), 'server/resources/templates/plugin/layout/templates'));
	
	instance.defaultthemedir = simpleportal.util.getServerPath(options.themedir || '/../../resources/theme');
	...
	instance.tempdir = simpleportal.util.getServerPath(options.tempdir|| path.join(__dirname, '/../../resources/._temp'));
	//if(serverInstance && serverInstance.getConfiguration())
	
	instance.systempublicappdir = options.systempublicappdir || path.join(simpleportal.getRootdir(), "server/resources/public");
	...
	instance.on("installed", function(options){ // to activate and update the plugin in to the plugin cache
		if(options.plugindir)
...
//	var tmpdir = util.getServerPath("../../dev-plugins");
	...
	var plugindirs = [];
	plugindirs.push(simpleportal.util.getServerPath('resources/public')); // server public directory
	...
	}else if(customplugindirs && typeof customplugindirs == "string" )
		plugindirs.push(util.getServerPath(customplugindir)); //@TODO remove it from production	
...
PluginLoaderMain.prototype.readPluginSetting = function(pluginsetting, plugindir, skipalt){
	...
	if(pluginsetting['public'] && pluginsetting.plugintype == 'theme')
		deployedroot = deployedroot + '/theme';
...
PluginLoaderMain.prototype.autoInstall = function(callback){
	...
	var resourcedownloaddir = instance.getServerInstance().getDataDirectory(instance.getServerInstance().getConfiguration("resources").downloaddir || '._downloads');
...
//	var uploaddir = instance.getServerInstance().getConfiguration("resources").uploaddir;
	var uploaddir = instance.getServerInstance().getDataDirectory(instance.getServerInstance().getConfiguration("resources").uploaddir||'._uploads');
	if(uploaddir){
...
var _installPlugin = function(instance, pluginresult, callback){
	...
	if(savedplugin && savedplugin.installed) {
		...
		if(instance.isNewerVersion(savedplugin, pluginsetting.version)){
...
var _movePlugin = function (instance, pluginresult, callback){
	...
	if(pluginsetting.plugintype == "uiplugin"){
		...
		if(pluginsetting["plugintype"] == "theme")
			deploydir = util.appendFilePath(deploydir, "theme");
	}
...
PluginLoaderMain.prototype.collectDependencies=function(pluginsetting){
	var dependencies = [];	
 ...
 * @param {callback} callback The callback to excecute when complete
 */
PluginLoaderMain.prototype.checkDependencies = function (pluginsetting, callback, saveresult){
	if(!pluginsetting)
		callback();
	else {
		var instance = this, 
		...
		dependencies = instance.collectDependencies(pluginsetting);
...
//		collectDependencies(pluginsetting.id, pluginsetting, dependencies);
		 ...
		 */
		if(pluginsetting && !pluginsetting.disabled 
			&& pluginsetting.npmdependencies && Object.keys(pluginsetting.npmdependencies).length > 0){
			// check you have 
			...
			for(var npmmindex in pluginsetting.npmdependencies){
				...
				if(!simpleportal.checkNodeModule(npmmodule)){
...
PluginLoaderMain.prototype.getTempPath = function(pluginsetting, file, file1){
	...
	return instance.getServerInstance().getTempDirectory(tmp_path);
}
 ...
 */
PluginLoaderMain.prototype.getUIPluginPath=function(subpath, resourceroot){
	var instance = this;
	if(!resourceroot)
		resourceroot = util.getServerPath(instance.uiplugindir);
	
	if(subpath)
		return util.appendFilePath(resourceroot, subpath);
	
	else return resourceroot;
		
}

PluginLoaderMain.prototype.getHtmlDirectory = function(uri, pluginsetting, deploydir){
	var instance = this,
		resourcepath,
		resourceabspath = null,
		pluginuri,
		plugindetailes, sourcedir;
	
	if(pluginuri) {
		resourcepath = uri.substring(uri.substring(1).indexOf(pluginuri) + pluginuri.length+1);
	...
	if(plugindetailes){
		sourcedir = plugindetailes.installedir||plugindetailes.sourcedir||deploydir;
		
		if(plugindetailes && plugindetailes.plugintype == 'theme' && uri.indexOf('/theme' + plugindetailes.themeuri) == 0){
			resourceabspath = uri.replace('/theme' + plugindetailes.themeuri, sourcedir);
...
//					pluginuri = uri.substring(0, uri.substring(1).indexOf("/"));
				...
				if(resourcepath.indexOf("html5/") == 0 && sourcedir.indexOf("/html5") == sourcedir.length-"html5/".length)
					resourcepath = resourcepath.substring(resourcepath.indexOf("html5/"));
				
				if(plugindetailes){
					
					if(plugindetailes.plugintype == "webapp")
						resourceabspath = util.appendFilePath(sourcedir, 'html5', resourcepath);
					else
						resourceabspath = util.appendFilePath(sourcedir, resourcepath);
				...
				if(!fs.existsSync(resourceabspath) && deploydir)
					resourceabspath = util.appendFilePath(deploydir, resourcepath);
				
				if(!fs.existsSync(resourceabspath) && resourcepath.indexOf("/uiplugin/") == 0){
					resourceabspath = util.getServerPath(instance.uiplugindir) + "/../" + resourcepath;
				}
			}
		}
	}
...
/**
 * To correct the dynamic path used in integration 
 */
 ...
 */
PluginLoaderMain.prototype.installMissingResources = function(pluginsource, options, callback){
	var instance = this,
		error;
	
	if(pluginsource.resources && pluginsource.resources.length > 0){
		if(pluginsource && pluginsource.missingresources && pluginsource.missingresources.length > 0){
			var cbc = pluginsource.missingresources.length;
			for(var i in pluginsource.missingresources){
				var missingdepid = pluginsource.missingresources[i];
				
				var missingresource = util.getJSONObject(pluginsource.resources, 'resourcename', missingdepid);
				
				installPluginResource(instance, pluginsource, missingresource, function(_error){
					if(_error){
						if(!error)
							error=[];
		...
		callback("no plugin resource found.");
...
PluginLoaderMain.prototype.installMissingDependencies = function(pluginsource, options, callback){
	...
	if(pluginsource && pluginsource.missingdependencies && pluginsource.missingdependencies.length > 0){
		...
		for(var i in pluginsource.missingdependencies){
			...
			instance.getServiceloader().getService("pluginsource").installPlugin({
					$or:[
					     {title:missingdepid}, {id:missingdepid}
					]
				}, options, function(_error, updateddata){
					if(_error){
						error.push(_error);
					}
					
					if(cbc-- == 1)
						callback(error && error.length > 0 ? error : null);
			});
		}
	}else
		callback()
...
var installPluginResource = function(instance, pluginsource, webresource, callback){
	if(pluginsource.plugintype == 'uiplugin')
		webresource.resourcefolder = util.appendFilePath(pluginsource.installeddir, webresource.resourcefolder);
	else { // dependencies must be installed in data diractory of the server.
		webresource.resourcefolder = instance.getDataDirectory(pluginsource, webresource.resourcefolder);
	
		// check whether the folder is available inside data folder
		simpleportal.util.checkDirSync(instance.getDataDirectory({id:pluginsource.plugintype}));
	}

	if(webresource && webresource.downloadlink && webresource.downloadlink.indexOf("/") == 0) {
		if(pluginsource.pluginstore && pluginsource.pluginstore.host)
			webresource.downloadlink = simpleportal.util.appendFilePath(pluginsource.pluginstore.host, webresource.downloadlink);
	}
	var tarargs = [];
	if(pluginsource && pluginsource.stripcomponent && pluginsource.stripcomponent > 0)
		tarargs.push('strip-components=1');
	
	var sr  = new Resourceinstaller(webresource);
	
	sr.install(callback, {deleteafter:true}, tarargs);
}
...
PluginLoaderMain.prototype.installDependencies = function(pluginsetting, callback, webresource){
	var instance = this,
		cbcount = 0, cberror=[];
	var tarargs = [];
	if(pluginsetting && pluginsetting.stripcomponent && pluginsetting.stripcomponent > 0)
		tarargs.push('strip-components=1');
	...
	if(pluginsetting && webresource) {
		...
		if(webresource && webresource.downloadlink && webresource.downloadlink.indexOf("/") == 0) {
			if(pluginsetting.pluginstore && pluginsetting.pluginstore.host)
				webresource.downloadlink = pluginsetting.pluginstore.host+ webresource.downloadlink;
		}
		...
		sr.install(function(error, data){
			...
			if(callback)
				callback(error, data)
		}, {deleteafter:true}, tarargs);
	} else if(pluginsetting){
		// let us check there is npm dependencies
		var totalcbcount = 0;
		if(pluginsetting.resources && pluginsetting.resources.length > 0)
			totalcbcount += pluginsetting.resources.length;

		if(pluginsetting.missingnpmdependencies){
			var modules  = pluginsetting.missingnpmdependencies;//Object.keys(pluginsetting.missingnpmdependencies);
				totalcbcount+=modules.length;
				
			for(var i in modules){
				var npmmodule = modules[i];
				var npmwithversion = pluginsetting.npmdependencies[npmmodule];
				
				util.installNpmModule((npmwithversion ? npmwithversion : npmmodule), function(error){
					if(error) cberror.push(error);
					
					if(cbcount++ == totalcbcount-1){
						if(cberror.length <= 0)
							cberror = null;
		...
		if(pluginsetting.resources && typeof pluginsetting.resources == "object"
			&& typeof pluginsetting.resources.length != "number")
		...
		if(pluginsetting.resources && pluginsetting.resources.length > 0){
			for(var i in pluginsetting.resources) {
				...
				if(webresource && webresource.stripcomponent && webresource.stripcomponent > 0)
					tarargs.push('strip-components=1');
...
var loadDependencies = function(instance, options){
	...
	if(autoinstall && pluginsetting && (
		...
		instance.installDependencies(pluginsetting, function(error){
			// @TODO make other checking synchronous
...
//			pluginsetting.themesetting.dependencies.push("sessiontimeoutapp");
	...
	if(isPluginActive(pluginsetting) && pluginsetting.plugintype == "webapp") {
		...
		simpleportal.util.getResources(pluginsetting.installeddir + "/resources/templates/pages", {
			resourcekey:pluginsetting.id, 
			includeroot:false, 
			extension:".html.ejs"
		}, function(error, webpages){
...
//			settingsobject.javascript = {};
	...
	if(dependantPlugin && dependantPlugin.plugintype == "webapp" && dependantPlugin.webappsetting.integration){
		...
		if(dependantPlugin && dependantPlugin.integration)integrationconfig= dependantPlugin.integration;
		if(dependantPlugin && dependantPlugin.plugintype == "webapp"){
			if(dependantPlugin.integration_minified)
				integrationconfig = dependantPlugin.integration_minified;
		}
		
		if(dependantPlugin && integrationconfig) {
			var versiontoload,
				defversiontoload = 'latest';
			
			if(Object.keys(integrationconfig).length == 1)
				defversiontoload = Object.keys(integrationconfig)[0];
			...
			if(dependentConfig.versions && dependentConfig.versions.length  > 0){
				...
				for(var i in dependentConfig.versions){
					...
					if(versiontoload){
						if(integrationconfig[versiontoload] && integrationconfig[versiontoload].javascript) {
							...
							var jdep = integrationconfig[versiontoload].javascript;
						...
						if(integrationconfig[versiontoload] && integrationconfig[versiontoload].css) {
							if(!settingsobject.css)
								settingsobject.css=[];
							
							var jcss = integrationconfig[versiontoload].css;
						...
						if(defversiontoload != versiontoload||dependentConfig.versions[i]){
							if(dependentConfig.id == plugindetails.id)
								settingsobject._dependencies['@' + versiontoload] = integrationconfig[versiontoload];
							else
								settingsobject._dependencies[dependantPlugin.id + '@' + versiontoload] = integrationconfig[versiontoload];
						} else
							settingsobject._dependencies[dependantPlugin.id] = integrationconfig[versiontoload];
					}
				}
...
//					settingsobject.javascript.dynamic = settingsobject.javascript.dynamic.concat(integrationconfig[versiontoload].javascript);
					var jdep = integrationconfig[versiontoload].javascript;
					if(typeof jdep == 'object' && typeof jdep.length == 'number'){
						settingsobject.javascript.dynamic = settingsobject.javascript.dynamic.concat(integrationconfig[versiontoload].javascript);
					}else{
				...
				if(integrationconfig[versiontoload] && integrationconfig[versiontoload].css) {
					...
					if(typeof settingsobject.css == "object" && typeof settingsobject.css.length == "number")
						settingsobject.css = settingsobject.css.concat(integrationconfig[versiontoload].css);
					else {
						if(!settingsobject.css.dynamic)
							settingsobject.css.dynamic = [];
						
						settingsobject.css.dynamic = settingsobject.css.dynamic.concat(integrationconfig[versiontoload].css);
					}
				}
...
/**
 * To inject a particular dependecy 
 * 
 */
PluginLoaderMain.prototype.injectDependency = function(plugindetails, dependentConfig, mobile, dependantPlugin){
	var instance = this, 
		dependantPlugin;
	
	if(!dependantPlugin && dependentConfig && dependentConfig.id == plugindetails.id)
		dependantPlugin = plugindetails;
...
//	
	if(dependantPlugin){
		var configkey;
		if(dependantPlugin && dependantPlugin.requireconfig && dependantPlugin.requireconfig.configkey)
			configkey = dependantPlugin.requireconfig.configkey;
			
		correctDynamicPath(dependantPlugin.integration, {uri:dependantPlugin.sourcedir, configkey:configkey});	
	}	
...
/**
 * To inject plugindependencies from dependent plugins
 * 
 */
PluginLoaderMain.prototype.getPluginDependencies = function(plugindetails){
	var instance = this,
		settingsfield = plugindetails.plugintype + 'setting',
		dependIds = [], 
		mobiledependIds = [];
	...
	if(plugindetails[settingsfield] && plugindetails[settingsfield].dependencies) {
	...
	return {
		dependIds:dependIds,
		mobiledependIds:mobiledependIds
	};
		...
		if(pluginsetting.integration){
			...
			for(var version in pluginsetting.integration) {
				if(pluginsetting.preference && pluginsetting.preference.minified){
					...
					if(!pluginsetting.integration_minified.javascript)
						pluginsetting.integration_minified.javascript = [];
					
					if(pluginsetting.integration_minified[version] && pluginsetting.integration_minified[version]["javascript"])
						pluginsetting.integration_minified["javascript"][version] = pluginsetting.integration_minified[version]["javascript"];
					else
...
//function getVersionAsNumber(versionnumber){
...
//			var binaryvalue = (Number(pad(1, ++index, true)))*prevbvalue;
//			
...
//		versionnumbervalue = Number(versionnumber) + 1;
//	else
//		versionnumbervalue = 1;
//	
//	return versionnumbervalue;
//}
//PluginLoaderMain.prototype.getVersionAsNumber = getVersionAsNumber;

PluginLoaderMain.prototype.isNewerVersion = function(pluginsetting, version){
	return util.getVersionAsNumber(pluginsetting.version) < util.getVersionAsNumber(version);
};
...
var pad = function (num, size, after) {
    var s = num + "";
    if(after)
    	while (s.length < size) 
    		s = s+ "0";
