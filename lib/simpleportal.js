...
var Constructor = function (options){
	 ...
	 * @static
	 */
	instance.rootdir = path.join(__dirname, '..');
	
	instance.resourcesdir = path.join(__dirname, '..', 'resources');
 ...
 */
Constructor.prototype.checkNodeModule = function(npmmodule){
	var instance = this;
	...
	for(var i in require.main.paths){
		try {
			...
			if(npmmodule.indexOf("@") == -1)
				resolvedpath = require.resolve(require.main.paths[i] + "/" + npmmodule);
			else
				resolvedpath = require.resolve(require.main.paths[i] + "/" + npmmodule.substring(0, npmmodule.indexOf("@")));
			return true;
		} catch(e) {}
	}
	return false;
...
Constructor.prototype.require = function(module){
...
//	    instance.views=Constructor.viewloader;
	...
	instance.getConfiguration = function(){
		//let us also search for various modules inside the simpleportal
		// simpleportal-core
		// simpleportal- webserver-resources
		// simpleportal-webserver-resources-templates
		simpleportal.util.getFileList(instance.getRootdir(), {root:instance.getRootdir(), filename:"plugin\.json"}, function(error, filelist){
			for(var i in filelist){
				var plugnpath = path.join(instance.getRootdir(), filelist[i].file);
				var plugininfo = simpleportal.util.readJSONFileSync(plugnpath);
				if(plugininfo.id){
					var subplugininfo = simpleportal.util.copyJSON({
						sourcedir:path.dirname(filelist[i].file)
					}, ['version', 'id', 'title', 'pluginstore'], plugininfo);
				}
			}
		});
