...
PluginLoader.loadPluginFromDirectory=function(plugindir, configuration, callback){
	...
	if(!fs.existsSync(plugindir))
		...
		var jsfiles = fs.readdirSync(plugindir).filter(function(v){ return !/\./.test(v)&&fs.existsSync(plugindir+'/'+v+'/plugin.json'); });
...
PluginLoader.loadPlugin = function(pluginsetting, callback){
	...
	loadPlugin(pluginsetting, pluginsetting.configuration, PluginLoader.dbInstance, callback);
}
