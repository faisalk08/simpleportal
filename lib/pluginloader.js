...
__loadPlugin = function(pluginsetting, configuration, dbInstance, callback){
	...
	if(pluginsetting.viewdir){
		console.log('CHECKING VIEW DIR -- '+ pluginsetting.installeddir + '/' + pluginsetting.viewdir +" <<<>>>"+ fs.existsSync(pluginsetting.installeddir + '/' + pluginsetting.viewdir));
		if(fs.existsSync(pluginsetting.installeddir + '/' + pluginsetting.viewdir)){
			...
			simpleportal.viewloader.loadViews({viewdir:pluginsetting.installeddir + '/' + pluginsetting.viewdir, views:viewconfig_}, function(){
				console.log('Finsihed view callbacks');
			});
		}
	}
 ...
 */
PluginLoader.readPluginSetting = function(pluginsettings, plugindir, skipalt){
