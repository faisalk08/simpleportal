...
StartupLoader.loadStartups = function(configuration, callback){
	...
	if(fs.existsSync(startupdir_)){
		...
		if (stats.isDirectory()) {
			loadStartups(startupdir_, false, callback, configuration.configuration||{});
		}else if(callback)
			callback();	
	}else if(callback)
		callback();
 ...
 * @param {} configuration
 */
function loadStartupByProps(startupprops, configuration){
	...
	if(!StartupLoader[startupprops.name]){
		...
		if(configuration)
			StartupLoader[startupprops.name].configuration=configuration;
	}
}
 ...
 * @return 
 */
function loadStartups(dir, includeindex, callback, configuration){
	var startupsfound = getStartups(dir, includeindex);
	
	logger.getInstance().info('Simple Portal - startuploader:getStartups', 'loadStartups -- ' + startupsfound.length);
	...
	if(startupsfound)
		for(var i in startupsfound){
			...
			loadStartupByProps(startupprops, configuration);
