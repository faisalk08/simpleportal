			...
			if(!error){
				...
				util.checkDir(restoredir +'/'+ inputFile, function(error){
					if(error)
						...
						var result = {};
						logger.info('Simpleportal -db', 'Exeuting the extracting tool command' + cmd_bckup);
							...
							if(error)
								...
								fs.readdirSync(restoredir +'/'+ inputFile).forEach(function(filename){
								    if (/\.bson$/.test(filename) && filemapping[filename]) {
								    	...
								    	logger.info('Simpleportal -db', 'Renaming file filename - '+ filename + ' -- to -- '+ newfilename);
								...
								logger.info('Simpleportal -db', 'Executing the restore tool command' + cmd);
