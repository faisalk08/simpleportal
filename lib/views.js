...
fs.lstat(viewdir, function(err, stats) {
	if (!err && stats.isDirectory()) {
		...
		logger.debug('Simple Portal-views', 'Views folder is not found in your root, using default..');
...
views.initRouter = function(router){
	logger.debug('Simple Portal -views', 'Initializing view routers');
    router.dispatch.addViewHandlers(views, viewUrl, views.call);
}

views.call = function(moduleName, method, request, response, callback){
	logger.error('Simple Portal -views', 'Request for view - '+ moduleName + ' method - '+ method +' -- is made');
